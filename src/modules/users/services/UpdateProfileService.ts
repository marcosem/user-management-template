...
class UpdateProfileService {
  ...
  public async execute({
    ...
    } else if (!user.admin && user_id !== profileID) {
      throw new AppError(
        'Only authenticated admins can update other players profile',
        401,
      );
    }
    ...
    if (!user.admin && admin !== undefined && profile.admin !== admin) {
      throw new AppError(
        'Only authenticated admins can update admin permissions',
        401,
      );
    } else {
      profile.admin = admin === undefined ? profile.admin : admin;
    }
